				<?php
					...
					if($this->uri->segment(3)){
						...
						//ambil riwatatttt
						$sgsgd = "select tb_register.tgl_awal_reg, tb_paket.nm_paket 
						from tb_register, tb_paket where tb_register.id_paket=tb_paket.id_paket 
						and tb_register.no_reg='".$pp[0]->no_reg."' order by tgl_awal_reg DESC limit 10";
						$sgsvv = $this->db->query($sgsgd);
						$sggsg = $sgsvv->result();
						//print_r($sggsg);
					}
					//print_r($_GET);
					...
					$gdbnmsnnm = "";
					if(isset($_GET['idol'])){
						$this->db->where("id", $_GET['idol']);
						$cekols = $this->db->get('api_booking');
						$getols = $cekols->row();
						$gsgb = explode('/', $getols->msg);
						$gdbnmsnnm = " - KODE BOOKING:" . $gsgb[1];
					}
				?>
				<table style="width:100%;" class="wedusbalapdaftar">
					<tr>
						<td style="width:230px;vertical-align:top;">
							...
							<?php
								...
								$cmb1 = $this->db->get('tb_user');
								$dnd = $cmb1->row();
								echo '<div align="center"><H4>LOKET PENDAFTARAN ';
								echo '<select onchange="gantilokerdaftar(this.value)"><option value="0">PILIH LOKET....</option>';
								foreach(range(1,3) as $gsjgh){
									$sell = "";
									if($gsjgh == $dnd->loket_pendaftaran){
										$sell = 'selected="true"';
									}
									echo '<option '.$sell.' value="'.$gsjgh.'">LOKET '.$gsjgh.'</option>';
								}
								echo '</select></H4></div>';
							?>
							<hr style="margin:5px;">
							<button type="button" onclick="panggilantrianpasienok()">Panggil Antrian</button>
							<hr style="margin:5px;">
							<b>Riwayat Kunjungan</b>
							<table style="width:100%;border-spacing:0;" class="wedusbalapdaftar">
							<tr style="background:#eeeeee;">
								<td><b>Tanggal</b><td>
								<td><b>Paket</b><td>
							</tr>
							<?php foreach($sggsg as $shh){ ?>
							<tr>
							<td><?=@date("Y-m-d", strtotime($shh->tgl_awal_reg))?><td>
								<td><?=@$shh->nm_paket?><td>
							</tr>
							<?php } ?>
							</table>
					...
					<input type="hidden" id="id_ol" name="id_ol" value="<?=@$getols->id?>">
					...
					<tr>
						<td colspan="5" style="background:#D1DFEC;"><div align="center"><b>KODE TRANSAKSI: <?=@$kod?> <?=@$tssdms?> <?=@$gdbnmsnnm?></b></div></td>
					</tr>
						...
						<td>
							<div align="left">
								...
								<?php 
									...
									$sel = "";
									if($getols){
										if($getols->id_paket == $va->id_paket){
											$sel = 'selected="true"';
										}
									}
								?>
							...
							<div align="left">
								...
								<?php 
									...
									foreach($cmb1 as $va){ 
										...
										if($getols){
											if($getols->cara_byr == $va->id_bayar){
												$sel = 'selected="true"';
											}
										}
								?>
...
<div id="modaluntukfarmasi" modal="true"  closed="true" maximizable="false" draggable="false" minimizable="false"  collapsible="true" class="easyui-window" title="Antrian Pasien" style="width:640px;height:600px;padding:10px;">
</div>
				<script type="text/javascript">
				
				
		
				function gantilokerdaftar(s){
					//alert(s);
					$.post('<?=@base_url('administrator/gantilokerdaftar')?>',{
						s:s,
					},function(result){ 
						//$('#panggilanloket').html(result);
					});
				}
				function panggilantrianpasienok(){
					$('#modaluntukfarmasi').window('open');
					$('#modaluntukfarmasi').panel({
						title: 'Panggil Antrian',
						href:'<?=@base_url($this->u1.'/panggilantriandata')?>',
					});
				}
