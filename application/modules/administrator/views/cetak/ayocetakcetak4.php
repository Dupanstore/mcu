	<?php
		...
		$pididok = is_stakesidtind_casis_nonpenerbad();
	...
	<table style="width:100%;border-spacing:0;">
		...
		<tr>
			<?php foreach(range(1,28) as $numbnew){ ?>
			<td><?=@$numbnew?></td>
			<?php } ?>
		</tr>
		</thead>
		<tbody>
			<?php 
				...
				foreach($ffds as $vdvb){
					...
					$judh = "select id_ins_tind_pem, id_tind_pem, val_stakes, stakes_tb, stakes_imt, stakes_anamnesa, stakes_tensi from tb_register_pemeriksaan where kode_transaksi='".$vdvb->kode_transaksi."' and  id_tind_pem IN (".$impidfr.") ";
					$keir = $this->db->query($judh);
					$dswew = $keir->result();
					//print_r($dswew);
					//die();
					if($dswew){
						foreach($dswew as $gdb){
							...
							if($gdb->id_tind_pem == "6564" OR $gdb->id_tind_pem == "6651"){
					...
					if(!empty($pemkesimpulan[$vdvb->id_reg]['A']['Kardiologi'])){
						$kesimpulanekg = $pemkesimpulan[$vdvb->id_reg]['A']['Kardiologi'];
					}
					...
					$warnstakesekg = "style='border-bottom:none;'";
					...
					if(trim(trim($stakesekg)) == "3"){
						$warnstakesekg = 'style="border-bottom:none;color:blue;font-weight:bold"';
						$parnstakesekg = 'color:blue;font-weight:bold';
					}
					...
					if(trim(trim($stakesekg)) == "4"){
						$warnstakesekg = 'style="border-bottom:none;color:red;font-weight:bold"';
						$parnstakesekg = 'color:red;font-weight:bold';
						$bintangekg = "*";
					}
					...
					$kesummmmok[$vdvb->id_reg][21] = trim(trim($stakesekg));
			...
			<tr>
				...
				<td <?=@$warnstakesekg?>><?=@$kesimpulanekg?><br /><?=@$bintangekg?></td>
				...
				<?php
					...
					if($getkeswaa == "3"){
						$warnagetkeswaa = 'style="border-bottom:none;color:blue;font-weight:bold"';
					}
					if($getaerostt == "3"){
						$warnagetaerostt = 'style="border-bottom:none;color:blue;font-weight:bold"';
					}
					if($statusakhirstakes == "3"){
						$warnastatusakhirstakes = 'color:blue;font-weight:bold"';
					}
					...
					if($getkeswaa == "4"){
						$warnagetkeswaa = 'style="border-bottom:none;color:red;font-weight:bold"';
					}
					if($getaerostt == "4"){
						$warnagetaerostt = 'style="border-bottom:none;color:red;font-weight:bold"';
					}
					if($statusakhirstakes == "4"){
						$warnastatusakhirstakes = 'color:red;font-weight:bold"';
					}
				?>
			...
			<tr >
				...
				<td style="border-top:none;<?=@$parnstakesekg?>"><?=@$stakesekg?></td>
				...
				<td style="border-top:none;<?=@$parnstakes21?>"></td>
			</tr>
			<?php } ?>
		</tbody>
	</table>
