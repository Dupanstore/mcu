	...
	.bordernone td{
		...
		<table style="width:100%;border-spacing:0;">
			<tr>
				...
				<td rowspan="2" style="vertical-align:middle;text-align:center;">NOMOR PESERTA</td>
		...
		<?php
				...
				foreach($abd as $bs){
					...
					$oiu = "select * from tb_resume_pasien where  kode_transaksi='".clean_data($bs->kode_transaksi)."' ";
					...
					//print_r($mdn);
						if($mdn){
							foreach($mdn as $sa){
								...
								}else if($sa->ket_resume == "periksatambahan"){
									$pemeriksaantambahan[$bs->id_reg][$sa->nama_kelainan] = $sa->isi_kelainan;
								}else{
					...
					$so=$nk++;
					$nourutfile = explode("/", $bs->no_filemcu);
		...
		<tr>
			...
			<td>
				<table class="bordernone">
					...
					?>
					<tr>
						<td><?=@$fvds->kesimpulan_kelainan ." ". $ghdhhjshjg?> 
				...
				<?=@$wedus->isi_kelainan?>
						<table>
							<?php foreach($pemeriksaantambahan[$bs->id_reg] as $scdd => $sdc){ 
								if($scdd == "Chamber-FI"){
									$scdd = "Chamber";
								}
								if(!empty($sdc)){
							?>
							<tr>
								<td style="border:none;"><?=@$scdd?></td>
								<td style="border:none;">:</td>
								<td style="border:none;"><?=@$sdc?></td>
							</tr>
								<?php } ?>
							<?php } ?>
						</table>
			...
			<td>
				<?php
					//print_r($stakessaranloop[$bs->id_reg]);
					echo max($stakessaranloop[$bs->id_reg]) ."<br />";
					echo $kondisipasien[$kondisipasienstakes[$bs->id_reg]['keterangan_sehat']] ."<br/><br/>";
					//ambil untuk kondisiiiii
