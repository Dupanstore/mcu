					...
					<tr>
						<td colspan="2">
							<table style="width:100%">
								...
								<tr>
									<td>PLT</td>
									<td><input type="text" style="width:100%" id="plt"  name="pemkhusus[plt]" value="<?=@$pemkhususnya['plt']?>"></td>
									<td colspan="2">%</td>
								</tr>
								<tr>
									<td colspan="4"><hr style="margin:5px;border:solid 1px #cccccc"></td>
						...
						if($fd->det_nilai_normal == "Y"){ 
							if($fd->det_type_pemeriksaan == "tetap"){
								if($fd->det_jenis_pemeriksaan == "combo"){
									...
									$exp = explode("*", $hdys);
									foreach($exp as $gdvfg){
										$gvksn[$gdvfg] = $gdvfg;
									}
									if(!isset($gvksn[$sipp[0]->hasilnya])){
										$exp[] = $sipp[0]->hasilnya;
									}
