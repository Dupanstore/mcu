<?php
		...
		$que 	 = "select id_dinas,  nm_dinas,  tipe_dinas, case when tipe_dinas='D' then 'Dinas' else 'Non Dinas' end as tipe_dinas_new, case when ila_medex='ila' then 'ILA' when ila_medex='medex' then 'MEDEX' when ila_medex='patiau' then 'PATI AU' when ila_medex='kemenkes' then 'KEMENKES' else '' end as set_ila_jmu from tb_dinas where 1=1 ";
		if (@!empty($_POST['cari'])){
