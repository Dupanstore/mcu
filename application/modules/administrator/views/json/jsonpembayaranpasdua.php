<?php
		...
		$que 	 = "select a.kode_transaksi, a.id_reg, a.no_filemcu, b.nip_nrp_nik, b.id_pas, b.nm_pas, b.no_tlp_pas from tb_register a, tb_pasien b where a.no_reg=b.no_reg ";
		...
		if(!isset($_POST['filter_tglawal'])){
			$que 	.= " and DATE_FORMAT(a.tgl_awal_reg, '%Y%m%d')='".date('Ymd', strtotime($_GET['filter_tanggalcari']))."' ";
		}
		/*if(@!empty($_POST['filter_paket'])){
			$que 	.= " and a.id_paket='".$_POST['filter_paket']."' ";
		}
		if(@!empty($_POST['filter_typejawatan'])){
			$que 	.= " and b.no_reg like '".$_POST['filter_typejawatan']."%' ";
		}
		if(@!empty($_POST['filter_jawatan'])){
			$que 	.= " and b.id_jawatan='".$_POST['filter_jawatan']."' ";
		}*/
		//$que 	.= " and a.konsul <> 'Y' ";
		//$que 	.= " and a.tgl_awal_reg BETWEEN '".date("Y-m-d", strtotime($_POST['filter_tglawal']))." 00:00:00' AND '".date("Y-m-d", strtotime($_POST['filter_tglakhir']))." 23:59:59' ";
		...
		if($ffds){
			foreach($ffds as $hj){
				...
				$hj->cetakkartu = '<a href="javascript:void(0)" onclick="cetakkartu(\''.$hj->kode_transaksi.'\', \''.$hj->id_reg.'\')"><img src="'.base_url('assets/img/payment.png').'" style="width:18px;"></a>';
				$gnhd[] = $hj;
			}
		}
			$object = new stdClass();
