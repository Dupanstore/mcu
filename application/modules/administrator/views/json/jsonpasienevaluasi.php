<?php
		...
		if(@!empty($_GET['filter_dinas'])){
			$que 	.= " and a.id_dinas_dua like '".$_GET['filter_dinas']."%' ";
		}
		...
		if($ffds){
			foreach($ffds as $hj){
					...
					$cekdataok = "select id_res from tb_resume_pasien where kode_transaksi='".$hj->kode_transaksi."'  and ket_resume='kesimpulansaran' ";
					$ghfdjjsgh = $this->db->query($cekdataok);
					$gdhsfsdyt = $ghfdjjsgh->row();
					//print_r($gdhsfsdyt);
					if($gdhsfsdyt){
