...
<?php
	...
	$getpemfinal = "select a.*, b.nomorbuku from tb_register_detailpemeriksaan a, tb_pemeriksaan b where a.id_pem_deb=b.id_pem and a.kode_transaksi='".clean_data($_GET['kode_transaksi'])."' and a.id_paket='".clean_data($_GET['id_paket'])."' and nomorbuku='12P4' ";
	$setpemfinal = $this->db->query($getpemfinal);
	$returnpemfinal = $setpemfinal->result();
	print_r($returnpemfinal);
													...
													<td>
														<div align="center">
															<?php
																...
																?>
															<?=@get_lang_hasil('p2', $_GET['lang'])?><br/><?=@$sghdfs->golongan?><br/><br/><br/><br/><?=@$sghdfs->nm_dok?><br />
															<?php if($_GET['lang'] == "en"){ ?>
															<?=@$sghdfs->pangkat_en?>
															<?php } else { ?>
															<?=@$sghdfs->pangkat?>
