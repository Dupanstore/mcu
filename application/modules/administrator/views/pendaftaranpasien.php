<div class="easyui-layout" data-options="fit:true" id="datadinas_layout1">
				   ...
				   data-options="singleSelect:true,fit:true,pagination:true,rownumbers:true,fitColumns:true" sortName="no_reg" sortOrder="ASC" toolbar="#tablemodalcaripasien_toolbar">
					<thead>
						<tr>
							...
							<th data-options="field:'paketlast'" width="30" sortable="true">Paket Terakhir</th>
	...
	<script type="text/javascript">
			<?php 
				$bsbdb = "";
				if(isset($_GET['idol'])){ 
				$bsbdb = "/". $_GET['idpas']."/?idol=". $_GET['idol'];
			?>
			<?php } ?>
			...
			$('#panel_daftar1').panel({
				href:'<?=@base_url($this->u1.'/ajaxpendaftaranpasien' . $bsbdb)?>',
			});
		...
		function importdataexcel(dek){
			...
			if(dek == "print"){
				window.open('<?=@base_url('administrator/cetakdataexcelmcu')?>/?typecetak='+dek+'&'+a);
			}else{
