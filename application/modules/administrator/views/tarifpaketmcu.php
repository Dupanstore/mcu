<div class="easyui-layout" data-options="fit:true" id="datakondisi_layout1">
        <div data-options="region:'west',split:true,footer:'#toolbar_paketmcu1'" title="" style="width:40%;background:#eeffff;">
				   ...
				   data-options="singleSelect:true,fit:true,rownumbers:true,fitColumns:true" sortName="kd_paket" sortOrder="ASC">
					<thead>
						<tr>
							...
							<th data-options="field:'casis_tniok'" width="20" sortable="true">Casis</th>
							<th data-options="field:'en_hasil'" width="20" sortable="true">EN</th>
							<th data-options="field:'autodinas'" width="20" sortable="true">Dinas</th>
							<th data-options="field:'autogrounded'" width="20" sortable="true">Grounded</th>
							<th data-options="field:'autoct'" width="20" sortable="true">Sertifikat</th>
	...
	<script type="text/javascript">
		...
		function rubahstatusautodinas(idp){
			$.post("<?=base_url($this->u1 .'/'. $this->u1 .'_action/rubahstatusautodinas/')?>", {
				idp:idp,
			}, function(response){	
				
			});
		}
		function rubahstatusautogrounded(idp){
			$.post("<?=base_url($this->u1 .'/'. $this->u1 .'_action/rubahstatusautogrounded/')?>", {
				idp:idp,
			}, function(response){	
				
			});
		}
		function rubahstatusautosert(idp){
			$.post("<?=base_url($this->u1 .'/'. $this->u1 .'_action/rubahstatusautosert/')?>", {
				idp:idp,
			}, function(response){	
