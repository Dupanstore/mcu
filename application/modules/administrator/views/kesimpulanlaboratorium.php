<div class="easyui-layout" data-options="fit:true" id="kesimpulanlaboratpasienpanel">
        <div data-options="region:'center',iconCls:'icon-ok'" title="">
            <div class="easyui-layout" data-options="fit:true">
               <table class="easyui-datagrid" id="tabelsatukesimpulanlaborat"
					   data-options="singleSelect:true,fit:true,pagination:true,rownumbers:true,fitColumns:true" sortName="tgl_awal_reg" sortOrder="DESC" toolbar="#tabelsatukesimpulanlaborat_tolbar">
						<thead>
							<tr>
								...
								<th data-options="field:'isihasil'" width="40" sortable="true">Hasil</th>
								<th data-options="field:'isikesimpulan'" width="40" sortable="true">Kesimpulan</th>
							</tr>
						</thead>
					</table>
					<div id="tabelsatukesimpulanlaborat_tolbar" style="padding-top:5px;padding-bottom:5px;height:auto">   
				<div align="right">
					<table style="width:70%">
					<tr>
						<td width="20%">
						<input class="easyui-searchbox" id="filter_keyword" data-options="prompt:'Masukkan Keyword',searcher:tampilkandatakesimpulanlaborat" style="width:100%;"></input>
						</td>
						<td width="10%">
			...
			<div id="modaltampilkandetailkesimpulanlaborat" inline="true" modal="true"  closed="true" maximizable="false" draggable="false" minimizable="false"  collapsible="false" class="easyui-window" title="Pencarian Referensi Registrasi" fit="true">
			</div>
        </div>
    </div>
	...
	function cellStylereval(value,row,index){
		...
		$('#tabelsatukesimpulanlaborat').datagrid({
				url: '<?=@base_url($this->u1.'/jsonpasienkesimpulanlaborat')?>/?filter_tglawal=<?=@date('Y-m-d')?>&filter_tglakhir=<?=@date('Y-m-d')?>&filter_jawatan=&filter_paket=&filter_typejawatan=&filter_keyword=',
			});
			
		
		$('#filter_tglawal').datebox({
			onSelect: function(date){
				tampilkandatakesimpulanlaborat();
			}
		});
		...
		function tampilkandatakesimpulanlaborat(){
			...
			$('#tabelsatukesimpulanlaborat').datagrid({
				url: '<?=@base_url($this->u1.'/jsonpasienkesimpulanlaborat')?>/?filter_tglawal='+filter_tglawal+'&filter_tglakhir='+filter_tglakhir+'&filter_jawatan='+filter_jawatan+'&filter_paket='+filter_paket+'&filter_typejawatan='+filter_typejawatan+'&filter_keyword='+filter_keyword,
			});
			setTimeout(function(){
			   $('#filter_keyword').textbox('textbox').focus().select();
			},20);
		...
		function rubahhasilkesimpulanlab(kd, id, isi, kets){
			$.post("<?=base_url($this->u1 .'/'. $this->u1 .'_action/rubahhasilkesimpulanlab/')?>", {
						id:id, isi:isi,kd:kd,kets:kets,
					}, function(response){	
						if(response == "simpan"){
							$('#tabelsatukesimpulanlaborat').datagrid('reload');
						}else{
							alert(response);
						}
					});
