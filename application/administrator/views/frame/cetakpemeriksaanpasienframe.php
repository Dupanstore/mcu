<?php
	...
	if($abs){
		$nhd = "select a.id_reg_pem, b.nm_tind, b.keterangan_tind, b.lantai_tind, b.ket_cetak_pemeriksaan_pasien, c.lantai, c.id_ins, c.nm_ins ";
		...
		//print_r($sba);
		foreach($sba as $st){
			$looppem[$st->lantai_tind][$st->id_ins] = $st->nm_ins;
			$looppem2[$st->lantai_tind][$st->id_ins] = $st->lantai;
			$looppem3[$st->lantai_tind][$st->id_ins][$st->id_reg_pem] = $st->nm_tind;
			$looppem4[$st->lantai_tind][$st->id_ins][$st->id_reg_pem] = $st->lantai_tind;
			$looppem5[$st->lantai_tind][$st->id_ins][$st->id_reg_pem] = $st->keterangan_tind;
				...
				<table id="tablenya" width="100%" style="margin-top:-10px;font-size:17px;font-family: calibri;" cellpadding="2px">
					...
					?>
								<tr>
									...
									<td><center><?=@$looppem2[$saq][$ff]?></center></td>
									<td><?=@$gg?></td>
									<td></td>
									<td width="12%">
									<div <?=@$kirinya?>><?=@$e++?> ............ </div>
							...
							<?php } ?>
							<?php 
								foreach($looppem3[$saq][$ff] as $op => $sq){ 
									$n++;
									$kirinya = "";
									$kanannya = 'style="border-left:0px"';
									if($n > 1){
							...
							?>
								<tr>
									<td style="border-left:0px solid"><?=@$u++?>.</td>
									<td><center><?=@$looppem4[$saq][$ff][$op]?></center></td>
									<td><?=@$sq?></td>
									<td><?=@$looppem5[$saq][$ff][$op]?></td>
									<td width="12%">
									<div <?=@$kirinya?>><?=@$e++?> ............ </div>
									</td>
									<td style="border-left:0px;">
