<div class="easyui-layout" data-options="fit:true" id="datakondisi_layout1">
        <div data-options="region:'west',split:true" title="Penunjang" style="width:350px;background:#eeffff;">
			<table class="easyui-datagrid" id="datapolipemeriksaantabel"  url="<?=@base_url($this->u1.'/jsongrouptambahpoli')?>"
		...
		<div data-options="region:'center',iconCls:'icon-ok'" title="">
            <div class="easyui-layout" data-options="fit:true">
                <table class="easyui-datagrid" id="tabel_grouppemeriksaan"
				   data-options="singleSelect:true,fit:true,pagination:true,rownumbers:true,fitColumns:true" sortName="orderdata" sortOrder="ASC" toolbar="#datakondisi_table1_toolbar">
					<thead>
						<tr>
							<th data-options="field:'kd_grouptindakan'" width="20" sortable="true">Kode</th>
							<th data-options="field:'nm_grouptindakan'" width="40" sortable="true">Nama</th>
							<th data-options="field:'orderdata',align:'center'" width="10" sortable="true">Order</th>
							<th data-options="field:'order_evalusi_group',align:'center'" width="20" sortable="true">Order Evaluasi</th>
						</tr>
					</thead>
				</table>
				<div id="datakondisi_table1_toolbar" style="padding:5px;height:auto">   
					<div> 
						<div align="right" style="margin:0 10px 0 0;">
							<input class="easyui-searchbox" data-options="prompt:'Masukkan Nama Group Pemeriksaan',searcher:datakondisicaridata" style="width:300px"></input>
						</div>
					</div>
				</div>
            </div>
		...
		<div data-options="region:'east',split:true,footer:'#datakondisi_panel1_toolbar',iconCls:'icon-lock'" title="Olah Data" style="width:400px;background:#eeffff;">
			...
			<form method="POST" id="datakondisi_form1" action="<?=base_url($this->u1 .'/'. $this->u1 .'_action/simpanupdatedatagrouppemeriksaan')?>">
	...
	<script type="text/javascript">
			...
			onSelect:function(index,row){  
				...
				$('#datakondisihidedua').hide();
				    $('#tabel_grouppemeriksaan').datagrid({
						url:'<?=@base_url($this->u1.'/jsondatagrouptindakan')?>/'+id,
					});
				$('#datakondisi_panel1').panel({
					href:'<?=@base_url($this->u1.'/ajaxdatagrouptindakan')?>/'+id,
				});  
		...
		function datakondisi_batal(){
			...
			$('#datakondisi_table1').datagrid('reload');
			$('#tabel_grouppemeriksaan').datagrid({
						url:'<?=@base_url($this->u1.'/jsondatagrouptindakan')?>/'+xxinstid,
					});
				$('#datakondisi_panel1').panel({
					href:'<?=@base_url($this->u1.'/ajaxdatagrouptindakan')?>/'+xxinstid,
				});   
		}
		$('#datakondisihidedua').hide();  
		$('#tabel_grouppemeriksaan').datagrid({  
			onSelect:function(index,row){  
				var id_ins = row.id_ins;
				var idgroup = row.id_grouptindakan;
				$('#datakondisihidesatu').hide();
				$('#datakondisihidedua').show();
				$('#datakondisi_panel1').panel({
					href:'<?=@base_url($this->u1.'/ajaxdatagrouptindakan')?>/'+id_ins+'/'+idgroup,
				});  
			}  
		}); 
		function datakondisi_simpandata(){
			$.messager.confirm('Konfirmasi', 'Anda yakin akan menyimpan data Group Pemeriksaan', function(r) {
				if (r){
		...
		function datakondisi_hapus(){
			$.messager.confirm('Konfirmasi', 'Anda yakin akan menghapus data Group Pemeriksaan', function(r) {
				if (r){
					var id = $('#id_grouptindakan').val();
					$.post("<?=base_url($this->u1 .'/'. $this->u1 .'_action/hapusdatagrouppemeriksaan/')?>", {
						id:id,
					}, function(response){	
						datakondisi_batal();
					});
