<div class="easyui-layout" data-options="fit:true" id="evaluasipasienpanel">
	...
	function cellStylereval(value,row,index){
		...
		function tampilkandataevaluasi(){
			...
			$('#tabelsatuevaluasi').datagrid({
				url: '<?=@base_url($this->u1.'/jsonpasienevaluasi')?>/?filter_tglawal='+filter_tglawal+'&filter_tglakhir='+filter_tglakhir+'&filter_jawatan='+filter_jawatan+'&filter_paket='+filter_paket+'&filter_typejawatan='+filter_typejawatan+'&filter_keyword='+filter_keyword,
			});
			setTimeout(function(){
			   $('#filter_keyword').textbox('textbox').focus().select();
			},20);
